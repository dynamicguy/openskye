...
public class ArchiveStoreDefinitionsCommand extends AbstractCrudCommand {
    ...
    public List<Field> getFields() {
        return FieldBuilder.start().add(new TextField("name")).add(new ReferenceField(Project.class)).add(new ReferenceField(ArchiveStoreInstance.class)).add(new TextField("implementation")).add(new PropertiesField("properties")).build();
    }
